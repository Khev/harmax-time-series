...
def run_dataset(ds: str, seed: int, epochs: int, n_exp: int):
    ...
    mins = int(elapsed // 60)
    secs = elapsed % 60
    logging.info("[%-15s] finished in %dm", ds, mins)
...
def main():
    ...
    ap.add_argument("--datasets", nargs="+", default=[
        ...
        "Coffee",
        "OliveOil",
        "GunPoint",           # classic motion data
        "ECG5000",            # larger ECG series
        "ItalyPowerDemand",   # real‐world electricity loads
        "Wafer",              # industrial sensor traces
        "FordA",              # engine noise recordings
    ])
