...
HIDDEN1   = 100
HIDDEN2   = 16
HARMONIC_N = 2      # exponent n in paper (they used n=1 for MLP section)
SEED = 42
#SEED = 14850
...
def main():
    ...
    # -------------------------------------------------------------------------
    # ADD-ON 1 ▸ 2×1 accuracy curves (train & test for each model)
    # -------------------------------------------------------------------------
    fig_acc, axs = plt.subplots(2, 1, figsize=(6, 8), sharex=True)

    for (idx, name) in enumerate(("Standard", "Harmonic")):
        axs[idx].plot(xs, history[name]["train_acc"], label="train", lw=1.3)
        axs[idx].plot(xs, history[name]["test_acc"],  label="test",  lw=1.3)
        axs[idx].set_ylabel("Accuracy")
        axs[idx].set_title(f"{name} – weight decay ={args.wd if name=='Standard' else 0}")
        axs[idx].set_ylim(0, 1.05)
        axs[idx].legend(loc="lower right", fontsize=8)

    axs[-1].set_xlabel("Epoch")
    fig_acc.suptitle("Modular addition (31-way) – accuracy curves", y=0.94, fontsize=12)
    fig_acc.tight_layout()
    fig_acc.savefig(FIG_DIR / "mod_addition_acc_2x1.png", dpi=120)

    # -------------------------------------------------------------------------
    # ADD-ON 2 (fixed) ▸ PCA of learned class representations
    # -------------------------------------------------------------------------
    from sklearn.decomposition import PCA

    def get_class_vectors(model: nn.Module) -> torch.Tensor:
        """
        Return the 31×16 matrix that represents each class in penultimate space.
        - Standard : final linear weights         (31,16)
        - Harmonic : learnt centres               (31,16)
        """
        if isinstance(model, HarmonicMLP):
            return model.centres.detach().cpu()
        else:  # Standard
            return model.out.weight.detach().cpu()   # shape (31,16)

    def pca_panel(model: nn.Module, ax, title: str):
        vecs = get_class_vectors(model).numpy()
        pca  = PCA(n_components=2)
        pcs  = pca.fit_transform(vecs)
        ev   = pca.explained_variance_ratio_.sum() * 100

        ax.scatter(pcs[:, 0], pcs[:, 1], s=28)
        for i, (x, y) in enumerate(pcs):
            ax.text(x, y, str(i), ha="center", va="center", fontsize=7)
        ax.set_aspect("equal");  ax.set_xlabel("PC0"); ax.set_ylabel("PC1")
        ax.set_title(f"{title}\nEV: {ev:.0f}%")

    fig_emb, axes_emb = plt.subplots(1, 2, figsize=(8, 4))
    pca_panel(models["Standard"],  axes_emb[0], "Standard")
    pca_panel(models["Harmonic"], axes_emb[1], "Harmonic")
    fig_emb.suptitle("Class-vector geometry after training", y=1.02)
    fig_emb.tight_layout()
    fig_emb.savefig(FIG_DIR / "mod_addition_class_vectors_pca.png", dpi=120)

    print(" • figures/mod_addition_class_vectors_pca.png")
